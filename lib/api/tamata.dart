...
class tamataItem {
  ...
  tamataItem(this.title, this.type, this.img, this.rateBase, this.strPrice,
...
searchTamata(query, pageNo) async {
  ...
  for (final ele in items!) {
    ...
    var itemLink = eleHtml
        .querySelector("div.product.details.product-item-details > strong > a")
        ?.attributes['href'];
    ...
    img ??= "NOT FOUND";
    itemsList.add(
        tamataItem(title, "Tamata", img, rateBase, strPrice, itemLink, price));
  }
  return itemsList;
}
